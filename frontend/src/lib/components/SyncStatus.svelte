...
<!-- {#if appState.syncing}
    <span class="sync-indicator syncing">{label}</span>
{:else if relative}
    <span class="sync-indicator idle">{label}</span>
{:else if appState.initialized}
    <span class="sync-indicator idle">{label}</span>
{/if} -->
<button class="sync-icon" aria-label="Sync now" onclick={() => appState.syncing || appState.globalError ? null : syncAll()} disabled={appState.syncing || !!appState.globalError}>
